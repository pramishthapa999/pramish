...
if (isset($_POST['submit'])) {
    if (!empty($_FILES['image']['name'])) {
        ...
        $folder = 'portfolio1/' . $file_name;

        // Insert the image details into the database
        $query = "INSERT INTO portfolio1 (file) VALUES ('$file_name')";
...
<body>
    <header>
        <h1>Precious Memories - Portfolio1 Gallery</h1>
    </header>
    ...
    <div class="image-gallery">
        ...
        $res = mysqli_query($conn, "SELECT * FROM portfolio1");
        if ($res) {
            while ($row = mysqli_fetch_assoc($res)) {
        ?>
                <div class="image-item">
                    <img src="portfolio1/<?php echo $row['file']; ?>" alt="portfolio1 Image">
                    <div class="actions">
                        <a href="update1.php?id=<?php echo $row['id']; ?>">
                            <button>Update</button>
                        </a>
                        <a href="delete1.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this image?');">
